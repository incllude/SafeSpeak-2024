...
class SSLModel(nn.Module):
    def __init__(self, device, path_to_pretrained_model):
        super(SSLModel, self).__init__()

        task_arg = argparse.Namespace(task='audio_pretraining')
        task = fairseq.tasks.setup_task(task_arg)
        cp_path = path_to_pretrained_model
        model, _, _ = fairseq.checkpoint_utils.load_model_ensemble_and_task([cp_path], task=task)
        self.model = model[0]
        self.device = device
        self.out_dim = 1024
        return
...
class AASIST2(nn.Module):
    def __init__(self, device, path_to_pretrained_wav2vec):
        super().__init__()
        self.device = device
        ...
        ####
        self.ssl_model = SSLModel(self.device, path_to_pretrained_wav2vec)
    ...
    def forward(self, x):
        ...
        for W in self.out_layer.parameters():
            W = F.normalize(W, p=2, dim=1)

        logits = self.out_layer(last_hidden_norm)
